...
if __name__ == '__main__':
    # create database   dataset[:, 0]和dataset[:, 1]
    data_len = 200
    ...
    train_data_ratio = 0.5  # choose 50% # Choose 80% of the data for testing
    ...
    # ----------------- train -------------------
    train_x_tensor = train_x.reshape(-1, 5, INPUT_FEATURES_NUM)  # 20,5,1 # set batch size to 5
    train_y_tensor = train_y.reshape(-1, 5, OUTPUT_FEATURES_NUM)  # set batch size to 5

    # transfer data to pytorch tensor
    train_x_tensor = torch.from_numpy(train_x_tensor)   # (20, 5, 1)
    train_y_tensor = torch.from_numpy(train_y_tensor)
    # test_x_tensor = torch.from_numpy(test_x)

    ### num_layers=1一层的时候1490个Epoch收敛，num_layers=2两层的时候653个Epoch收敛
